...
Event.observe(window, 'load', function () {
    Object.extend(window, {
        ...
        showActionMessage: function (message, type, trace) {
            ...
            if( trace ) {
                $('actionmessage').innerHTML += '<span id="showtrace">?</span>';
                $('actiontrace').setOpacity(0.0);
                $('actiontrace').show();
                $('actiontrace').innerHTML = trace;
                new Effect.Opacity('actiontrace', {duration:1, from:0, to:1.0});
                $('actiontrace').onclick = function( ){$('actiontrace').hide();};
            }
            $('actionmessage').className = type;
            $('actionmessage').onclick = function( ){$('actionmessage').hide();};

            new Effect.Opacity('actionmessage', {duration:1, from:0, to:1.0});
                
            if( type == 'success' || type == 'error' )
                setTimeout(function(){ 
                    new Effect.Opacity('actionmessage', {duration:3, from:1.0, to:0}); 
...
PopUpWindow.prototype = {
    ...
    show: function (params) {
        ...
        if( this.name == "params_edit")
            jQuery( "#"+this.name +"_container").css("z-index" , 100002 );
    }
};
...
entityList.prototype = {
    ...
    autocomplete: null,
    templateData: null,
    foreignElementTemplate:null,

    addElement: function (value,label) {
        //modifying checked flag in the elements list
        this.source.each(function (item) {
            if (item.value==value) {
                item.checked=true;
            }
        });

        //removing added element from the autocomplete list
        var autocompleteSource = jQuery( "#foreign-input-"+this.fieldName ).autocomplete('option','source');
        autocompleteSource = jQuery.grep(autocompleteSource, function(item) {
            return item.value != value;
        });
        jQuery( "#foreign-input-"+this.fieldName ).autocomplete('option','source',autocompleteSource);

        //making template
        this.templateData.label = label;
        this.templateData.value = value;
        jQuery("#foreign-container-"+this.fieldName).append(this.foreignElementTemplate.evaluate(this.templateData));
    },

    removeElement: function(value) {
        //addinng removed element to the autocomplete list and modifying checked flag in the elements list
        var autocompleteSource = jQuery( "#foreign-input-"+this.fieldName ).autocomplete('option','source');
        this.source.each(function (item) {
            if (item.value==value) {
                item.checked=false;
        ...
        //removing label and hidden input
        jQuery('#foreign-element-container-'+this.fieldName+'-'+value).remove()
    },

    initialize: function (fieldName,arrayName,source,foreignElementTemplate) {
        this.fieldName=fieldName;
        this.foreignElementTemplate = foreignElementTemplate;
    ...
    processAutocomplete: function () {
        var t=this;
        //preparation for autocomplete
        jQuery( "#foreign-input-"+this.fieldName ).autocomplete({
            source: this.source,
            minLength:0,
            position: {collision: "flip fit"},

            select: function( event, ui ) {
                t.addElement(ui.item.value, ui.item.label);
                ui.item.value='';
            },
        ...
        //filling selected elements

        for(i=0,len=this.source.length;i<len;i++){
            if (this.source[i].checked) {
                this.addElement(this.source[i].value,this.source[i].label);
            }
        }
    }
}