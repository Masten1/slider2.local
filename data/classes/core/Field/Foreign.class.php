...
class Field_Foreign extends Field_Int {
    ...
    public function asAdorned() {
        $manager = fvManagersPool::get( $this->entityName );
    ...
    function getList( fvRoot $entity ) {
        $foreigns = fvManagersPool::get( $this->entityName )->getAll( $this->where );
    ...
    function asEntity() {
        if ( !isset(self::$cache[$this->entityName][$this->get()]) ){
            self::$cache[$this->entityName][$this->get()] = fvManagersPool::get( $this->entityName )->getByPk( $this->get(), true );
        }
