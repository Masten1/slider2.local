...
class fvConfig{
    ...
    protected function checkConfig( $fileName ){
        ...
        if( is_file( $cacheName ) ){
            if( filectime( $cacheName ) < filectime( $fileName ) )
                return false;
            include( $cacheName );
            return false;
        } else{
    ...
    protected function putToCache( $fileName, $configData ){
        $data = '<?php $this->mergeConfig(&$this->config, ' . var_export( $configData, true ) . ");";
    ...
    function Load( $fileName, $fullPath = false ){
        ...
        if( !file_exists( $configFile ) )
            throw new Exception( "Can't load config file '" . $configFile . "'. Terminating" );
    ...
    function getModuleName( $mod ){
        return $this->get( "modules." . $mod . ".name" );
    }

    public function langRoot(){
        return "{$this->get("dir_web_root")}{$this->getCurrentLang()}/";
    }

    /**
     * returns current lang
     * @return string ru|en|ua|etc...
     */
    public function getCurrentLang(){
        return ( defined( "lang" ) ) ? lang : null;
    }

    public function loadFromDatabase(){
        $configs = AConfig::getManager()->getAll();
        ...
        foreach( $configs as $row ){
            ...
            $m = &$this->config;
            foreach( explode( ".", $row->keyword ) as $level ){
                if( !isset( $m[$level] ) )
                    $m[$level] = Array();
                $m = &$m[$level];
            }
