...
<p style="color: #666; font-size: 10px; font-style: italic;" id="autocomp"> — Начните вводить название</p>

<div id="foreign-container-{$field->getKey()}" class="entity-wrapper">
    {foreach from=$field->get() item="ex"}
        <div class="selectedList" id="foreign-element-container-categories-{$ex->getPk()}">{$ex}<a href="javascript: void(0);" onclick="window.entityListcategories.removeElement({$ex->getPk()}); return false;"><img width="16" height="16" src="/backend/img/delete.png" title="удалить"></a><input type="hidden" value="{$ex->getPk()}" name="data[main][{$field->getKey()}][]"></div>
    {/foreach}
</div>
