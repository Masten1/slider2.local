        ...
        <div id="scrolobble">
            <div id="header">
                ...
                {if $fvConfig->get('access.enable')}
                    ...
                    <div id="hello" class="text">{if $LoggedManager}Добро пожаловать, <B>{$LoggedManager->name}! [<a href="/backend/">на главную</a>] [<a href="{$fvConfig->get('access.do_logout')|urlto}">выйти</a>]{/if}</div>
                {/if}
            </div>
            
            {if $smarty.const.debug}<script>var debug = true;</script>{/if}
