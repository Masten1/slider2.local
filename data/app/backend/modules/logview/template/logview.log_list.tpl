...
<div class="filter_form" style="margin-bottom: 10px; float: left;">
<h3>фильтр</h3>
    ...
    {html_options name=filter[managerId] id=managerId options=$UserManager->htmlSelect(null, 'любой', 'groupId = 1') selected=$filter_manager_id style="width: 150px; float: left; display: block;"}
    <br clear="all">
    <label for="message" style="width: 270px;">сообщение (может быть долго):</label> <input type="text" id="message" name="filter[message]" value="{$filter_message}" style="width: 300px;">
...
<tr>
    ...
    <td>
        {if $Log->editLink}
            <a href="{$Log->editLink}" onclick="go('{$Log->editLink}'); return false;"><img src="{$fvConfig->get('dir_web_root')}img/report_go.png" border="0" width="16" height="16" title="Перейти к объекту"></a>
        {/if}
    </td>
</tr>
{/foreach}
