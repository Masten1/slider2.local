    ...
    <div class="form">
        ...
        <div style="width: 50%; float: left;">
            <fieldset>
                ...
                <label for="name">Название</label> <input type="text" id="name" name="mg[name]" value="{$UserGroup->name}" /><br/>
                ...
                <input type="checkbox" name="mg[isDefault]" value="1" id="m_default_group" {if $UserGroup->isDefault->get()}checked="true"{/if}>
                <label for="m_default_group" class="checkbox">Группа по умолчанию</label> <br />

                <input type="checkbox" name="mg[isActive]" value="1" id="m_isActive" {if $UserGroup->isActive->get()}checked="true"{/if}>
                <label for="m_isActive" class="checkbox">Активность пользователя в группе</label> <br />
                            ...
                            <ul class="acls">
                                {foreach item=acl from=$acl_group key=acl_name}
                                    ...
                                    {if $acl_name ne 'name'}{if is_array($acl)}
                                                ...
                                                <ul class="acls">
                                                    {foreach item=acl_chld from=$acl key=acl_chld_name}
                                                        {if $acl_chld_name ne 'name'}
                                                            <li><input type="checkbox" name="mg[permissions][]" value="{$acl_chld_name}" id="mg_permitions_{$acl_name}_{$acl_chld_name}" {if in_array($acl_chld_name, $UserGroup->permissions->asArray())}checked="true"{/if} onchange="$('mg_permitions_{$acl_name}').checked = false;">
                                        ...
                                        {else}
                                            <li><input type="checkbox" name="mg[permissions][]" value="{$acl_name}" id="mg_permitions_{$acl_name}" {if in_array($acl_name, $UserGroup->permissions->asArray())}checked="true"{/if}>
